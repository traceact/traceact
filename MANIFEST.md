...
| `traceact/sinks.py` | `JsonlSink` (file, thread-safe, `max_bytes` rotation), `ConsoleSink` (stdout), `AsyncSink` (background-thread wrapper, bounded queue), `SqliteSink` (local database), `HttpSink` and `OtlpSink` (network delivery via stdlib `urllib`, guarded by `_netguard`, `network_policy` warn/enforce/off), `ObjectStoreSink` (batches traces into newline-delimited JSON objects, gzip optional) with `S3Backend` (any S3-API store, SigV4-signed with stdlib `hmac`/`hashlib`, same `_netguard`/`network_policy` guard). |
| `traceact/_netguard.py` | The shared outbound-network guard: destination classification (private/link-local/reserved/multicast/unspecified rejected by default, loopback always permitted), plain-`http://` policy, redirect-refusing opener. Used by `HttpSink`, `OtlpSink`, `ObjectStoreSink`'s backend (`S3Backend`), and the viewer's focus-hook forward. Touches DNS (`socket.getaddrinfo`). |
...
| `tests/test_object_store.py` | `ObjectStoreSink` and `S3Backend`: SigV4 signing against the aws-sig-v4-test-suite get-vanilla vector, path-style PUT over the wire, batching and time-ordered keys, gzip option, failure counting, `network_policy` enforce, and delivery of a sub-threshold batch when the wrapping `AsyncSink` is closed or flushed. |
